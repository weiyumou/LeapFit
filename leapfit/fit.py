...
class LogisticFit:
    ...
    def centred_students(self, data: StepData) -> tuple[pd.Series, float]:
        ...
        Reference coding leaves the KC intercepts meaning "for the reference
        student", which is an arbitrary choice. Moving to ``mean(theta) = 0``
        makes it "for the average student" instead. This is a slide along the
        flat direction of :meth:`~leapfit.design.Design.identify`, so every
        fitted value is unchanged — the caller must add the same shift to the
        KC intercepts.
